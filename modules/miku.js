...
exports.install = function() {
  F.on('ready', function() {
    if(F.settings.bot && F.settings.bot.id && F.settings.bot.token) {
      client = new Discord.Client();
      game = new Discord.Collection();
      ...
      client.on('message', msg => {
        ...
        if(msg.content.charAt(0) === F.settings.bot.trigger) {
          ...
          if (msg.content.includes("gameshuffle") || msg.content.includes('gs')) {
            ...
            if(!voiceChannel){
              ...
              .then (function(games) {
                ...
                msg.channel.send(`${msg.author}-san, ${F.settings.games.responses[Math.floor(Math.random()*F.settings.games.responses.length)].replace('$message$', `${selected.name}`)}`);
              });
        ...
        if(user.voiceChannel) {
          ...
          user.voiceChannel.members.every((member) => {
            ...
            member.roles.every((role) => {
              ...
              if(role.color == F.settings.games.roleColor) {
      ...
      client.login(F.settings.bot.token);
    } else {
      LOG(`Miku disabled!`);
    }
  });
